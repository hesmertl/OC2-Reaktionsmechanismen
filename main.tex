...
Das Enolat wird durch benachbarte elektronenziehende Gruppen stabilisiert, dann ist die Deprotonierung des $\alpha$-H leichter.\\

\noindent Es gilt die Faustregel:
\textit{Eine} CH\textit{-Säure wird von einer äquimolaren Menge Base zum größten Teil deprotoniert, wenn der} pK$_a$ \textit{der} CH\textit{-Säure vom} pK$_a$ \textit{der konjugierten Säure des basischen Reagenzes übetroffen wird.}  
\[
	pK_a(\ce{BaseH}^+)>pK_a(\ce{CH-S\textnormal{"a}ure})
\]
Basenstärke: t-BuLi $>$ s-BuLi $>$ n-BuLi $>$ PhLi, $\dots$ $>$ LTMP $>$ LDA $>$ LiHMDS
...
\subsection{alkalische Esterhydrolyse}
Auch als Esterverseifung bezeichnet.
\begin{figure}[H]
	\centering 
	\includegraphics[width=0.9\linewidth]{Bilder/Esterverseifung.png}
\end{figure}
Die alkalische Esterhydrolyse ist irreversibel und läuft daher vollständig ab.
Im Sauren hingegen ist die Esterhydrolyse reversibel.
